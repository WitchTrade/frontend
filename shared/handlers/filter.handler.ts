...
const FilterHandler = (
  ...
  setMarketType?: (marketType: MARKET_TYPE) => void
) => {
  ...
  const [itemFilterValues, setItemFilterValues] = useState<ItemFilterValues>(
    createDefaultItemFilter(type)
  )

  useEffect(() => {
    if (
      (!queryLoaded || usernameChanged) &&
      router.isReady &&
      type !== FILTER_TYPE.NEWTRADE &&
      type !== FILTER_TYPE.IGNORELIST
    ) {
      const searchString =
        typeof router.query.searchString === 'string'
          ? router.query.searchString
          : ''
      const tradeableOnly =
        typeof router.query.tradeableOnly === 'string' &&
        router.query.tradeableOnly === 'true'
          ? true
          : false
      const newOnly =
        typeof router.query.newOnly === 'string' &&
        router.query.newOnly === 'true'
          ? true
          : false
      const newMarketType =
        typeof router.query.marketType === 'string' &&
        router.query.marketType === '1'
          ? MARKET_TYPE.WISH
          : MARKET_TYPE.OFFER
      ...
      if (typeof router.query.orderDirection === 'string') {
        orderDirection = orderDirectionValues.find(
          (odv) => odv.key === router.query.orderDirection
        )
      }
      let itemCharacters
      if (typeof router.query.itemCharacter === 'string') {
        itemCharacters = router.query.itemCharacter
          .split('-')
          .map((id) =>
            itemCharacterValues.find((icv) => icv.id === parseInt(id))
          )
          .filter((val) => val)
      }
      let itemSlots
      if (typeof router.query.itemSlot === 'string') {
        itemSlots = router.query.itemSlot
          .split('-')
          .map((id) => itemSlotValues.find((isv) => isv.id === parseInt(id)))
          .filter((val) => val)
      }
      let itemEvents
      if (typeof router.query.itemEvent === 'string') {
        itemEvents = router.query.itemEvent
          .split('-')
          .map((id) => itemEventValues.find((iev) => iev.id === parseInt(id)))
          .filter((val) => val)
      }
      let itemRarities
      if (typeof router.query.itemRarity === 'string') {
        itemRarities = router.query.itemRarity
          .split('-')
          .map((id) => itemRarityValues.find((irv) => irv.id === parseInt(id)))
          .filter((val) => val)
      }
      let inventory
      if (typeof router.query.inventory === 'string') {
        inventory = inventoryValues.find(
          (i) => i.key === router.query.inventory
        )
      }
  ...
  useEffect(() => {
    ...
    const query: { [key: string]: string | boolean | undefined } = {
      searchString:
        itemFilterValues.searchString !== ''
          ? itemFilterValues.searchString
          : undefined,
      tradeableOnly:
        itemFilterValues.tradeableOnly !== false
          ? itemFilterValues.tradeableOnly
          : undefined,
      newOnly:
        itemFilterValues.newOnly !== false
          ? itemFilterValues.newOnly
          : undefined,
      orderBy:
        itemFilterValues.orderBy.key !== orderByValues[0].key
          ? itemFilterValues.orderBy.key
          : undefined,
      orderDirection:
        (type === FILTER_TYPE.MARKET &&
          itemFilterValues.orderDirection.key ===
            orderDirectionValues[0].key) ||
        (type !== FILTER_TYPE.MARKET &&
          itemFilterValues.orderDirection.key === orderDirectionValues[1].key)
          ? itemFilterValues.orderDirection.key
          : undefined,
      itemCharacter:
        itemFilterValues.itemCharacter.length > 0
          ? itemFilterValues.itemCharacter.map((ic) => ic.id).join('-')
          : undefined,
      itemSlot:
        itemFilterValues.itemSlot.length > 0
          ? itemFilterValues.itemSlot.map((is) => is.id).join('-')
          : undefined,
      itemEvent:
        itemFilterValues.itemEvent.length > 0
          ? itemFilterValues.itemEvent.map((ie) => ie.id).join('-')
          : undefined,
      itemRarity:
        itemFilterValues.itemRarity.length > 0
          ? itemFilterValues.itemRarity.map((ir) => ir.id).join('-')
          : undefined,
      inventory:
        itemFilterValues.inventory.key !== inventoryValues[0].key
          ? itemFilterValues.inventory.key
          : undefined,
  ...
  const filterItems = () => {
    let filteredItems = items.filter((item) => {
      const searchString = item.name
        .toLowerCase()
        .includes(itemFilterValues.searchString.toLowerCase())
      const tradeableOnly = itemFilterValues.tradeableOnly
        ? item.tradeable
        : true
      ...
      if (itemFilterValues.itemCharacter.length > 0) {
        itemCharacter =
          itemFilterValues.itemCharacter.some(
            (is) => is.key === item.tagCharacter
          ) ||
          (item.tagCharacter === null &&
            itemFilterValues.itemCharacter.some((is) => is.key === 'none'))
      }
      let itemSlot = true
      if (itemFilterValues.itemSlot.length > 0) {
        itemSlot = itemFilterValues.itemSlot.some(
          (is) => is.key === item.tagSlot
        )
      }
      let itemEvent = true
      if (itemFilterValues.itemEvent.length > 0) {
        itemEvent =
          itemFilterValues.itemEvent.some((is) => is.key === item.tagEvent) ||
          (item.tagEvent === null &&
            itemFilterValues.itemEvent.some((is) => is.key === 'none'))
      }
      let itemRarity = true
      if (itemFilterValues.itemRarity.length > 0) {
        itemRarity = itemFilterValues.itemRarity.some(
          (ir) => ir.key === item.tagRarity
        )
      }

      let inventoryFilter = true
      if (inventory.id && itemFilterValues.inventory.key == 'owned') {
        inventoryFilter = inventory.inventoryItems.some(
          (ii) => ii.item.id === item.id
        )
      } else if (
        inventory.id &&
        itemFilterValues.inventory.key == 'duplicateown'
      ) {
        inventoryFilter = inventory.inventoryItems.some(
          (ii) => ii.item.id === item.id && ii.amount > 1
        )
    ...
    if (type === FILTER_TYPE.MARKET) {
      if (trades) {
        const filteredTrades = trades.filter((t) =>
          filteredItems.some((item) => item.id === t.item.id)
        )
      ...
      if (type === FILTER_TYPE.NEWTRADE) {
        filteredItems = filteredItems.filter(
          (i) => i.tradeable && !trades?.some((t) => t.item.id === i.id)
        )
      }
      if (type === FILTER_TYPE.IGNORELIST) {
        filteredItems = filteredItems.filter(
          (i) => i.tradeable && i.tagSlot !== 'ingredient'
        )
      }
  ...
  return {
    inventory,
    totalItemCount:
      type === FILTER_TYPE.MARKET && trades ? trades.length : items.length,
    filteredItems,
    loadedItems,
    loadMoreItems,
    hasMoreItems,
