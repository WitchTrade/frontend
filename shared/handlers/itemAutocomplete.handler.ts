...
const ItemAutocompleteHandler = (setItem: (item: Item | undefined) => void) => {
  ...
  useEffect(() => {
    ...
    setFoundItemRefs(
      Array(filteredItems.length)
        .fill(null)
        .map((_, i) => foundItemRefs[i] || createRef())
    )
  }, [filteredItems])

  useEffect(() => {
    if (
      keyNavigation.cursor >= 0 &&
      foundItemRefs[keyNavigation.cursor].current
    ) {
      if (
        keyNavigation.positionInView === 1 &&
        keyNavigation.direction === 'up'
      ) {
        foundItemRefs[keyNavigation.cursor].current.scrollIntoView({
          block: 'start',
        })
      } else if (
        keyNavigation.positionInView === 6 &&
        keyNavigation.direction === 'down'
      ) {
        foundItemRefs[keyNavigation.cursor].current.scrollIntoView({
          block: 'end',
        })
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.keyCode === 38 && keyNavigation.cursor > 0) {
      ...
      const newPositionInView =
        keyNavigation.positionInView > 1
          ? keyNavigation.positionInView - 1
          : keyNavigation.positionInView
      setKeyNavigation({
        cursor: newCursor,
        item: filteredItems[newCursor],
        positionInView: newPositionInView,
        direction: 'up',
      })
    } else if (
      e.keyCode === 40 &&
      keyNavigation.cursor < filteredItems.length - 1
    ) {
      const newCursor = keyNavigation.cursor + 1
      const newPositionInView =
        keyNavigation.positionInView < 6
