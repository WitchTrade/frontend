...
const SearchHandler = () => {
  ...
  const [loadedTrades, setLoadedTrades] = useState<
    SearchOffer[] | SearchWish[]
  >([])

  const [searchFilterValues, setSearchFilterValues] =
    useState<SearchFilterValues>(createDefaultSearchFilter())
  const [searchOrderValues, setSearchOrderValues] = useState<SearchOrderValues>(
    createDefaultSearchOrder()
  )
    ...
    if (!queryLoaded && router.isReady && items.length > 0) {
      const itemId =
        typeof router.query.itemId === 'string'
          ? parseInt(router.query.itemId, 10)
          : undefined
      const wishlistOnly =
        typeof router.query.wishlistOnly === 'string' &&
        router.query.wishlistOnly === 'true'
          ? true
          : false

      let orderBy
      if (typeof router.query.orderBy === 'string') {
        orderBy = tradeableOrderByValues.find(
          (obv) => obv.key === router.query.orderBy
        )
      }
      let orderDirection
      ...
      if (typeof router.query.itemCharacter === 'string') {
        itemCharacter = itemCharacterValues.find(
          (icv) => icv.key === router.query.itemCharacter
        )
      }
      let itemSlot
      if (typeof router.query.itemSlot === 'string') {
        itemSlot = itemSlotValues.find(
          (isv) => isv.key === router.query.itemSlot
        )
      }
      let itemEvent
      if (typeof router.query.itemEvent === 'string') {
        itemEvent = itemEventValues.find(
          (iev) => iev.key === router.query.itemEvent
        )
      }
      let itemRarity
      if (typeof router.query.itemRarity === 'string') {
        itemRarity = tradeableItemRarityValues.find(
          (irv) => irv.key === router.query.itemRarity
        )
      }
      let inventory
  ...
  useEffect(() => {
    setLoadedTrades(
      trades.slice(0, loadedTrades.length > 20 ? loadedTrades.length : 20)
    )
  }, [trades])
  ...
  useEffect(() => {
    ...
    const query: { [key: string]: string | boolean | undefined } = {
      itemId: searchFilterValues.item
        ? searchFilterValues.item.id.toString()
        : undefined,
      orderBy:
        searchOrderValues.orderBy.key !== tradeableOrderByValues[0].key
          ? searchOrderValues.orderBy.key
          : undefined,
      orderDirection:
        searchOrderValues.orderDirection.key !== orderDirectionValues[0].key
          ? searchOrderValues.orderDirection.key
          : undefined,
      itemCharacter:
        searchFilterValues.itemCharacter.key !== itemCharacterValues[0].key
          ? searchFilterValues.itemCharacter.key
          : undefined,
      itemSlot:
        searchFilterValues.itemSlot.key !== itemSlotValues[0].key
          ? searchFilterValues.itemSlot.key
          : undefined,
      itemEvent:
        searchFilterValues.itemEvent.key !== itemEventValues[0].key
          ? searchFilterValues.itemEvent.key
          : undefined,
      itemRarity:
        searchFilterValues.itemRarity.key !== tradeableItemRarityValues[0].key
          ? searchFilterValues.itemRarity.key
          : undefined,
      inventory:
        searchFilterValues.inventory.key !== inventoryValues[0].key
          ? searchFilterValues.inventory.key
          : undefined,
      wishlistOnly:
        searchFilterValues.wishlistOnly !== false
          ? searchFilterValues.wishlistOnly
          : undefined,
    }
