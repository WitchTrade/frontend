...
const sortedGameModes = ['HaS', 'Mobi', 'HaH', 'Imp', 'FaP']

const sortedWeekDays = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
...
const useStatsHandler = () => {
  ...
  useEffect(() => {
    ...
    Chart.defaults.borderColor = themeService.hexToRgbA(
      theme.colors.chartText,
      0.1
    )

    // PIE CHART
    // REGION
    const playerDistRegionStats = stats.filter(
      (stat) => stat.statGroup === 'w_distribution_region'
    )
    ...
    // PIE CHART
    // MODE
    const playerDistModeStats = stats.filter(
      (stat) => stat.statGroup === 'w_distribution_mode'
    )
    playerDistModeStats.sort((a, b) => {
      return (
        sortedGameModes.indexOf(a.dataset) - sortedGameModes.indexOf(b.dataset)
      )
    })
    ...
    // LINE CHART
    // TOTAL
    let playerCountTotalStats = stats.filter(
      (stat) => stat.statGroup === 'w_playercount'
    )

    const last2Hour = new Date()
    last2Hour.setUTCHours(
      last2Hour.getUTCHours() - (last2Hour.getUTCHours() % 2),
      0,
      0,
      0
    )
    ...
    playerCountTotalStats.sort((a, b) => {
      const aNum = parseInt(
        a.label.substring(a.label.indexOf(' '), a.label.indexOf(':')),
        10
      )
      const bNum = parseInt(
        b.label.substring(a.label.indexOf(' '), b.label.indexOf(':')),
        10
      )
      return aNum - bNum
    })
    playerCountTotalStats.sort((a, b) => {
      return (
        sortedWeekDays.indexOf(a.label.substring(0, a.label.indexOf(' '))) -
        sortedWeekDays.indexOf(b.label.substring(0, b.label.indexOf(' ')))
      )
    })

    const playerCountTotalSplitIndex =
      playerCountTotalStats.findIndex(
        (cr) =>
          cr.label.substring(0, cr.label.indexOf(' ')) === last2HourWeekDay &&
          parseInt(
            cr.label.substring(cr.label.indexOf(' '), cr.label.indexOf(':')),
            10
          ) === last2Hour.getUTCHours()
      ) + 1
    ...
    // LINE CHART
    // REGION
    let playerCountRegionStats = stats.filter(
      (stat) => stat.statGroup === 'd_playercount_region'
    )
    ...
    const playerCountRegionSplitIndex =
      playerCountRegionStats.findIndex(
        (cr) =>
          parseInt(cr.label.substring(0, cr.label.indexOf(':')), 10) ===
          lastHour
      ) + 3
    ...
    const playerCountRegionChart = new Chart(playerCountRegion.current, {
      type: 'line',
      data: {
        labels: playerCountRegionStats
          .filter((cr) => cr.dataset === 'EU')
          .map((cr) => cr.label),
        datasets: [
          {
            label: 'EU',
            data: playerCountRegionStats
              .filter((cr) => cr.dataset === 'EU')
              .map((cr) => cr.value),
            backgroundColor: theme.colors.chartColor1,
            borderColor: theme.colors.chartColor1,
    ...
    // LINE CHART
    // MODE
    let playerCountModeStats = stats.filter(
      (stat) => stat.statGroup === 'd_playercount_mode'
    )
    ...
    const playerCountModeIndex =
      playerCountModeStats.findIndex(
        (cr) =>
          parseInt(cr.label.substring(0, cr.label.indexOf(':')), 10) ===
          lastHour
      ) + 3
    ...
    const playerCountModeChart = new Chart(playerCountMode.current, {
      type: 'line',
      data: {
        labels: playerCountModeStats
          .filter((cr) => cr.dataset === 'Hide and Seek')
          .map((cr) => cr.label),
        datasets: [
          {
            label: 'Hide and Seek',
            data: playerCountModeStats
              .filter((cr) => cr.dataset === 'Hide and Seek')
              .map((cr) => cr.value),
            backgroundColor: theme.colors.chartColor1,
            borderColor: theme.colors.chartColor1,
          },
          {
            label: 'Mobification',
            data: playerCountModeStats
              .filter((cr) => cr.dataset === 'Mobification')
              .map((cr) => cr.value),
            backgroundColor: theme.colors.chartColor2,
            borderColor: theme.colors.chartColor2,
          },
          {
            label: 'Hunt a Hag',
            data: playerCountModeStats
              .filter((cr) => cr.dataset === 'Hunt a Hag')
              .map((cr) => cr.value),
            backgroundColor: theme.colors.chartColor3,
            borderColor: theme.colors.chartColor3,
          },
          {
            ...
            data: playerCountModeStats
              .filter((cr) => cr.dataset === 'Imposturous (WIP)')
              .map((cr) => cr.value),
          ...
          },
          {
            ...
            data: playerCountModeStats
              .filter((cr) => cr.dataset === 'Fill a Pot (WIP)')
              .map((cr) => cr.value),
  ...
  const fetchServerStats = async (): Promise<Stat[]> => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/stats/witchitserver`
    )
