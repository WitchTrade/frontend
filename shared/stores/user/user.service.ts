...
export class UserService {
  ...
  public fetchCurrentUser() {
    return authService.request(
      `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/users/current`
    )
  }

  public fetchSyncSettings() {
    return authService
      .request(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/users/syncSettings`)
      .pipe(
        tap({
          next: async (res) => {
            const json = await res.json()
  ...
  public updateAccountSettings(user: Partial<User>) {
    return authService
      .request(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/users`, {
        method: 'PUT',
        headers: {
          'content-type': 'application/json',
        },
  ...
  public changePassword(oldPassword: string, password: string) {
    ...
    // login user and save info to store
    return authService
      .request(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/users/password`, {
        method: 'PUT',
        headers: {
          'content-type': 'application/json',
        },
  ...
  public fetchProfile(username: string) {
    return fromFetch(
      `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/users/get/${username}`
    ).pipe(
      tap({
        next: async (res) => {
          if (!res.ok) {
