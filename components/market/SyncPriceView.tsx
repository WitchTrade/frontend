        ...
        <div className="inline-block max-w-lg p-6 pb-16 my-8 overflow-auto text-left align-middle transition-all transform bg-wt-surface-dark shadow-xl rounded-2xl border-4 border-wt-success">
          <div className="h-full flex flex-col justify-between">
            ...
            <div className="flex flex-wrap justify-center items-center mt-3">
              {prices.filter(p => !excludeIds.includes(p.id) &&
                (
                  (type === MARKET_TYPE.OFFER && p.forOffers) ||
                  (type === MARKET_TYPE.WISH && p.forWishes) ||
                  (type === MARKET_TYPE.SYNC && p.forSync && !p.priceKey.startsWith('dynamic'))
                )).map(p => (
            ...
            <div className="flex flex-wrap justify-center items-center mt-3">
              {type === MARKET_TYPE.SYNC &&
                prices.filter(p => !excludeIds.includes(p.id) && p.forSync && p.priceKey.startsWith('dynamic')).map(p => (
                  <Tooltip key={p.id} text={p.displayName}>
                    <div className={`m-1 h-14 w-14 rounded-lg p-1 cursor-pointer ${price?.id === p.id ? 'bg-wt-accent' : ''}`} onClick={() => { setPrice(p); setDialogOpen(false); }}>
                      <Image className="rounded-lg" src={`/assets/images/prices/${p.priceKey}.png`} height={56} width={56} quality={100} alt={p.displayName} />
                    </div>
